...
const GameHostSetup: React.FC = () => {
  // Use try-catch to handle potential context errors during initial render
  try {
    const { 
    ...
    // If an error occurs during initialization (e.g., context not available),
    // redirect to the home page
    console.error("Error in GameHostSetup:", error);
    return (
      <div className="min-h-screen bg-gradient-to-b from-primary/10 to-accent/10 flex flex-col justify-center items-center">
        <div className="text-center p-8 bg-white/80 backdrop-blur-sm rounded-lg shadow-lg">
          <h2 className="text-2xl font-bold text-primary mb-4">שגיאה בטעינת המשחק</h2>
          <p className="mb-6">אירעה שגיאה בטעינת המשחק. אנא נסה שוב מדף הבית.</p>
          <Link 
            to="/"
