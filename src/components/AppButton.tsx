...
const AppButton = React.forwardRef<HTMLButtonElement, AppButtonProps>(({
  ...
  return <button className={cn('relative inline-flex items-center justify-center font-medium transition-all duration-200 shadow-sm', variants[variant], sizes[size], fullWidth ? 'w-full' : '', isDisabled && 'opacity-50 cursor-not-allowed hover:bg-opacity-100', className)} disabled={isDisabled} ref={ref} {...props}>
        {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
        {!loading && leftIcon && <span className="mr-2">{leftIcon}</span>}
        <span className="my-0 mx-0 text-center text-lg">{children}</span>
